...
<body class="gray-bg">
	<div class="panel-body">
        ...
        <div class="row">
			  ...
			  <div class="col-lg-2">
				<div class="input-group">
					<span class="input-group-addon">老师姓名</span>
					...
					<form id="form_user" method="post" action="reserveStudent.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>入校时间：</td>
								<td>
								<input placeholder="入校时间" id="entrytime" name="entrytime" 
								  class="laydate-icon form-control layer-date" readonly="readonly"/>
									</td>
								<td>&nbsp;&nbsp;</td>
								<td>创建时间：</td>
					...
					<form id="form_teacher" method="post" action="reserveTeacher.htm">
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								 <td>入校时间：</td>
								<td>
								<input placeholder="入校时间" id="entrytime1" name="entrytime" 
								  class="laydate-icon form-control layer-date" readonly="readonly"/>
									</td> 
								</tr>
						</table>
    ...
    laydate({
	 ...
	 laydate({
	  ...
	  laydate({
	        elem: '#entrytime1', //目标元素。由于laydate.js封装了一个轻量级的选择器引擎，因此elem还允许你传入class、tag但必须按照这种方式 '#id .class'
	        event: 'focus', //响应事件。如果没有传入event，则按照默认的click
	        format: 'YYYY-MM-DD'// 日期格式
	    });
	    		...
	    		$('#modal_user_echarts').modal({backdrop: 'static', keyboard: false});
	    			...
	    			$.ajax({
	    			    ...
	    			    success:function(res){
	    			    	...
	    			    	if(res.success){
		    		    		//循环集合
		    			    	res.list.forEach(function(studentVo){
